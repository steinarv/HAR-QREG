---
title: "Implementation and testing the HAR-QREG model i R"
...
This R Markdown document will give a quick introduction on implementing the HAR-QREG model of [Haugom et al. (2016)](http://www.sciencedirect.com/science/article/pii/S1544612315001385) in R and backtesting the Value at Risk(VaR) predictions.
Predicting VaR is essentially predicting quantiles and hence quantile regression is a natural choice when modelling VaR. The HAR-QREG model is a quantile regression model using short term, medium term and long term historical volatility (square root of 1 day, 5 days and 20 days average squared returns) as explanatory variables. This is of course heavily innspired by the HAR-RV model of [Corsi (2009)](https://www.scopus.com/record/display.uri?eid=2-s2.0-62849101579&origin=inward&txGid=a69b94b190edf2833685a89bf157a243).

Beneath we will predict the 5 % quantile (VaR) on daily returns from the DAX index, using a rolling window of 1000 days.

## Code
For implementing and testing the model we only need to load/install to packages. The [GAS](https://www.rdocumentation.org/packages/GAS/versions/0.2.4) package has some nice financial data in adittion to implentation of the [Conditional Coverage test](https://www.jstor.org/stable/2527341?seq=1#page_scan_tab_contents) and the [Dynamic Quantile test](http://www.simonemanganelli.org/Simone/Research_files/caviarPublished.pdf). And of course the [quantreg](https://cran.r-project.org/web/packages/quantreg/index.html) package for quantile regression.
...
We will continue with the DAX index in this example and proceed with creating the three explantory variabels with use of the [filter function](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/filter.html). Notice that by removing the last observation and include an *NA* in the beginning of the vectors, we get lagged observations. This means that the volatility estimates on a given row, is not estimated using the acctual return on that row.
...
After removing the rows with missing observations we may estimate and predict 5% VaR with a rolling window of 1000 trading days within a for-loop. For comparisment we also include the empirical 5% quantile of the 1000-day window (often refferd to as historical simulation). 
